...
$query3 = mysqli_query($conn, "SELECT * FROM appointments WHERE DATE(appointment_date) = CURDATE()");
...
<head>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            let dateInput = document.getElementById("dte"); // Ensure this matches your date input field

            if (dateInput) {
                let originalDate = dateInput.value; // Store the original value (loaded from PHP)

                let today = new Date().toISOString().split("T")[0]; // Get today's date in YYYY-MM-DD format
                dateInput.setAttribute('min', today); // Set the minimum selectable date
                ...
                dateInput.addEventListener("change", function() {
                    ...
                    if (selectedDate > maxAllowedDate) {
                        Swal.fire({
                            ...
                            dateInput.value = originalDate; // Restore the previous valid date
                        });
                    }
                });
            } else {
...
<body>
    ...
    <div class="content">
        <div class="header-list">
            <div class="left pt-4">
                <h4 class="fw-bold">Today Appointments List</h4>
                <p style="margin-top:10px;"><a href="./Dashboard.php"> HealthCare </a> > <span> Today Appointments
                    </span></p>
            </div>
            <div class="right">
                        ...
                        <?php while ($appointment = mysqli_fetch_assoc($query3)) { ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    $payment = mysqli_fetch_assoc($query7);
