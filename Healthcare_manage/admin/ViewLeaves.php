...
$query3 = mysqli_query($conn, "SELECT * FROM doctor_leave");
...
if (isset($_REQUEST["leaveid"])) {
    ...
    if ($status == "on") {
        $status = "Approve";
        $lu = "UPDATE doctor_leave SET `status` = '$status' WHERE leave_id = $leaveid ";
        mysqli_query($conn, $lu);
    } else {
        $status = "Disapprove";
        $lu = "UPDATE doctor_leave SET `status` = '$status' WHERE leave_id = $leaveid ";
        mysqli_query($conn, $lu);
                        ...
                        <?php while ($leave = mysqli_fetch_assoc($query3)) { ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <?php
                                    $docid = $leave["doctor_id"];
                                    $fetch_doctor = mysqli_query($conn, "SELECT * FROM doctors WHERE doctor_id=$docid");
                                    $doctor = mysqli_fetch_assoc($fetch_doctor);
                                ...
                                <td class="text-center">
                                    ...
                                    <?php } elseif ($leave["status"] == "Cancelled") { ?>
                                        <div class="badge bg-warning text-dark">Doctor Cancelled</div>
                                    <?php } else { ?>
                                    ...
                                    if ($LeaveDate > $currentDate) {
                                        if ($leave["status"] == "Cancelled") { ?>
                                            <div class="form-check form-switch d-flex justify-content-center">
                                                <input class="form-check-input fs-4 p-1 " type="checkbox" disabled />
                                            </div>
                                        <?php } else { ?>
                                            <form method="post" class="d-flex justify-content-center">
                                                ...
                                                <div class="form-check form-switch d-flex justify-content-center">
                                                    <input class="form-check-input fs-4 p-1 " type="checkbox"
                                                        id="switch<?= $leave["leave_id"] ?>" onchange="this.form.submit()"
                                                        name="status" <?php if ($leave["status"] == "Approve") { ?> checked <?php } ?> />
                                                </div>
                                            </form>
                                        <?php } ?>
                                    <?php } else { ?>
                                        <div class="form-check form-switch d-flex justify-content-center">
                                            <input class="form-check-input fs-4 p-1 " type="checkbox" <?php if ($leave["status"] == "Approve") { ?> checked <?php } ?> disabled />
                                        </div>
                                    <?php } ?>
                                </td>
                            </tr>
